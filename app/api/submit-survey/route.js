...
export async function POST(req) {
  try {
    ...
    if (!surveyId || !answers) {
      return new Response(
        JSON.stringify({
          error: "Faltan datos necesarios para procesar la encuesta",
        }),
        { status: 400 }
      );
    }
    ...
    for (const question of survey_questions.flatMap(
      ...
      if (question.optionalIf) {
        // Ejemplo: { pregunta: 107, opciones: ["TIENDA DE ABARROTES", ...] }
        const { pregunta, opciones } = question.optionalIf;
        const respuestaCondicion = answers[pregunta];
        if (opciones && opciones.includes(respuestaCondicion)) {
          // Si la condición se cumple, la pregunta es opcional: saltar validación
          continue;
        }
      }
      ...
      switch (question.questionType) {
        ...
        case "percentages_sum_100":
          if (
            ...
            Object.values(userAnswer).reduce(
    ...
    const saved = await prisma.response.create({
      data: { answers: validAnswers, encuestaId: parseInt(surveyId) }, // Save the survey response
    });
    return new Response(
      JSON.stringify({
        success: true,
